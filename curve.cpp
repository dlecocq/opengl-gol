...
string curve::get_geom_shader() {
	string program = read_file("shaders/curve.geom.front") + func + read_file("shaders/curve.geom.back");
	if (layout & X_LOG) {
...
void curve::gen_shader() {
	...
	glProgramParameteriEXT(pc, GL_GEOMETRY_INPUT_TYPE_EXT,  GL_LINES);
	//glProgramParameteriEXT(p, GL_GEOMETRY_OUTPUT_TYPE_EXT, GL_POINTS);
	glProgramParameteriEXT(pc, GL_GEOMETRY_OUTPUT_TYPE_EXT, GL_LINE_STRIP);
	/*
	GLint temp;
	glGetIntegerv(GL_MAX_GEOMETRY_OUTPUT_VERTICES_EXT, &temp);
	//*/
	glProgramParameteriEXT(pc, GL_GEOMETRY_VERTICES_OUT_EXT, 40);
